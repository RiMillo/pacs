...
namespace Geometry{
  ...
  AbstractPolygon::AbstractPolygon(std::vector<unsigned int> const & v, Vertices * ptr, bool check) : idpt(v), ptrv(ptr)
  {
  ...
  void AbstractPolygon::showMe(std::ostream & out)const
  {
    if (this->size()==0){
      ...
      out<<"    X     " << "   Y    "<<std::endl;
      for (auto const & i : idpt)
	out<<(ptrv->at(i)).x()<<" " << (ptrv->at(i)).y()<<std::endl;
    }
  ...
  void AbstractPolygon::checkConvexity()
  {
	...
	p = myV[idpt[i]];
	// ! next point
	Point2D tmp = myV[(idpt[i]+1) % myV.size()];
	v = tmp - p;
	//! next next point
	u = myV[(idpt[i]+2) % myV.size()];
  ...
  Polygon::Polygon(std::vector<unsigned int> const & v, Vertices * ptr): AbstractPolygon(v,ptr) {}
  ...
  */
  double Polygon::area() const{
    ...
    for (decltype(siz) i=0; i<siz;++i){
      ...
      // of objects
      Point2D const & p1(ptrv->at(idpt[i]));
      // Other point
      Point2D const & p2(ptrv->at(idpt[(i+1)%siz]));
      Point2D const & p0(ptrv->at(idpt[(i-1)%siz]));
  ...
  // ********************* SQUARE **********************
  Square::Square(std::vector<unsigned int> const & v, Vertices * ptr): AbstractPolygon(v,ptr,false) 
  {
    this->isconvex=true;
    if(v.size() != 4){
    ...
    double l1=distance(ptr->at(idpt[1]),ptr->at(idpt[0]));
    double l2=distance(ptr->at(idpt[2]),ptr->at(idpt[3]));
  ...
  double Square::area() const{
    ...
    Point2D v(this->ptrv->at(idpt[1])-this->ptrv->at(idpt[0]));
    Point2D w(this->ptrv->at(idpt[2])-this->ptrv->at(idpt[0]));
  ...
  Triangle::Triangle(std::vector<unsigned int> const & v, Vertices * ptr):AbstractPolygon(v,ptr,false){
    this->isconvex=true;    

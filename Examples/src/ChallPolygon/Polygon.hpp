...
namespace Geometry
{
  ...
  */
  ...
  public:
     ...
     */
    AbstractPolygon(std::vector<unsigned int> const & v, Vertices * ptr, bool check=true);
    ...
    */
	std::vector<unsigned int>::size_type size() const {return idpt.size();}
    ...
    */
	...
	//void print_pt() {
		//for(auto it=idpt.begin();it!=idpt.end();++it)
			//std::cout << *it <<" ";
		//std::cout<<std::endl;
	//}
  protected:
    //Vertices vertexes;
	std::vector<unsigned int>	idpt;
	Vertices *					ptrv;
    bool isconvex;
  ...
  public:
    ...
    Polygon(std::vector<unsigned int> const & v, Vertices * ptr);
    //! Destructor
    virtual ~Polygon(){};
    /*!
  ...
  {
  public:
    Square(std::vector<unsigned int> const & v, Vertices * ptr);
  ...
  {
  public:
    Triangle(std::vector<unsigned int> const & v, Vertices * ptr);
